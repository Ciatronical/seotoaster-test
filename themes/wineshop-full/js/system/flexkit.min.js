...
television="googletv viera smarttv internet.tv netcast nettv appletv boxee kylo roku dlnadoc roku pov_tv hbbtv ce-html".split(" ");for(a=0;a<television.length;){if(c(television[a]))return!0;a++}return!1};a.portrait=function(){return 1<window.innerHeight/window.innerWidth};a.landscape=function(){return 1>window.innerHeight/window.innerWidth};a.noConflict=function(){window.device=b;return this};c=function(a){return-1!==p.indexOf(a)};h=function(a){return g.className.match(new RegExp(a,"i"))};d=function(a){h(a)||
...
function tapButton(a,b){var d=document.getElementsByClassName(a);if(window.Hammer)Hammer.each(d,function(a){(new Hammer(a,{domEvents:!0})).on("tap",b)});else for(var g=0;g<d.length;g++)window.addEventListener?d[g].addEventListener("click",b,!1):window.attachEvent&&d[g].attachEvent("click",b,!1)};device.type&&(replaceSelector(":hover|:active",".touch",!1),$("body").on("touchstart",function(a){$(a.target).addClass("touch")}).on("touchend touchmove",function(a){$("*").removeClass("touch")}));
if(device.type){if(!document.getElementById("loader")){var preLoad,horz,vert;preLoad=document.createElement("div");preLoad.id="loader";horz=document.createElement("span");horz.className="horz";preLoad.appendChild(horz);vert=document.createElement("span");vert.className="vert";preLoad.appendChild(vert);document.body.appendChild(preLoad)}window.onbeforeunload=showLoading;window.onload=hideLoading}function showLoading(a){$("#loader").removeClass("hidden")}
function hideLoading(a){a.preventDefault();$("#loader").addClass("hidden")};function tableResponsive(){$("table.responsive").each(function(){var a=$("tr",this),b=[];$("th",this).each(function(){var a=$(this).text();b.push(a)});$.each(b,function(d,b){a.find("td:eq("+d+")").prepend('<span class="table-head">'+b+"</span>")})})};var sheet,rule,selectors,newSelectorRule,newRule,j,f;
function replaceSelector(a,b,d){var g=new RegExp(a+"\\b");d=d?new RegExp(d+"\\b"):d;try{for(var c=0;c<document.styleSheets.length;c++)if(sheet=document.styleSheets[c],null!=sheet.cssRules&&0!=sheet.cssRules.length)for(j=0;j<sheet.cssRules.length;j++)if(rule=sheet.cssRules[j],rule.type===CSSRule.STYLE_RULE)changeRule(rule,g,b,a,rule.type,d);else if(rule.type===CSSRule.MEDIA_RULE)for(f=0;f<rule.cssRules.length;f++)changeRule(rule.cssRules[f],g,b,a,rule.type,d)}catch(e){alert(e)}}
